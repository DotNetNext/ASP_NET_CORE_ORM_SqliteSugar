...
namespace SQLiteSugar
{
    ...
    public class SqlSugarClient : SqlHelper
    {
        ...
        /// <returns>T的集合</returns>
        public List<T> SqlQuery<T>(string sql, SqliteParameter[] pars)
        {
        ...
        /// <returns>T的集合</returns>
        public List<T> SqlQuery<T>(string sql, List<SqliteParameter> pars)
        {
        ...
        public object Insert<T>(T entity, bool isIdentity = true) where T : class
        {
            ...
            foreach (PropertyInfo prop in props)
            {
                ...
                if (!isIdentity || identities.Any(it => it.Value.ToLower() != propName.ToLower()))
                {
                    ...
                    var par = new SqliteParameter(SqlSugarTool.ParSymbol + propName, val);
        ...
        public bool Delete<T>(T deleteObj)
        {
            ...
            foreach (var pkName in pkNames)
            {
                ...
                SqliteParameter par = new SqliteParameter(pkName.GetSqlParameterName(), pkValue);
                pars.Add(par);
                SqlSugarTool.SetParSize(par);
            }
